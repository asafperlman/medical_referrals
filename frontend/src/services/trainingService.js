...
// שנה את הגדרת הבסיס לנתיבי אימון
const TRAINING_URL = `${API_PREFIX}/trainings`;

// שנה את ה-interceptor
api.interceptors.request.use(
  (config) => {
    const token = localStorage.getItem('access_token'); // שינוי מ-'token' ל-'access_token'
    if (token) {
...
// יצירת מופע axios עם הגדרות בסיסי
